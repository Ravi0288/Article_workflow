...
def create_new_object(source, row, note):
    ...
    settings.MEDIA_ROOT = 'data/metadata/ARTICLES'
    try:
        # Create new record
...
def update_exisiting_object_single_file(source, row):
    ...
    destination = settings.ARTICLE_ROOT + '/' + (source.replace('ARCHIVE/','').replace('TEMP/',''))
...
def update_exisiting_object_zipped_file(source, row):
    ...
    destination = settings.ARTICLE_ROOT + '/' + destination
